...
class StrategyValidator:
    ...
    def walk_forward_analysis(self, stock_y, stock_x, market_index,
        ...
        for i in range(n_folds):
            ...
            try:
                # Test on test period using ConservativeSystem
                test_system = ConservativeSystem()
                test_results = test_system.run_backtest(
                    test_y, test_x, test_market,
                    train_period=min(126, len(test_y)//2),
                    entry_z_normal=entry_z,
                    exit_z_normal=exit_z,
                    entry_z_volatile=entry_z + 0.5,
                    exit_z_volatile=max(0.1, exit_z - 0.1),
                    min_hold_days=3
                )
        ...
        if len(results) == 0:
            ...
            self.validation_results['walk_forward'] = pd.DataFrame()
            return pd.DataFrame()
    ...
    def monte_carlo_simulation(self, n_simulations=1000, confidence_level=0.95):
        ...
        # Filter out zero-pnl trades to get meaningful signal
        trades = trades[trades['pnl'] != 0].copy()

        if len(trades) < 5:
            print(" Not enough non-zero trades for Monte Carlo")
            return None
        
        actual_returns = trades['pnl'].values
        actual_total = actual_returns.sum()
        actual_sharpe = (actual_returns.mean() / actual_returns.std() * 
                         np.sqrt(252 / max(trades['duration_days'].mean(), 1))) if actual_returns.std() > 0 else 0
        ...
        for _ in range(n_simulations):
            ...
            sharpe = (shuffled.mean() / shuffled.std() * 
                      np.sqrt(252 / max(trades['duration_days'].mean(), 1))) if shuffled.std() > 0 else 0
        ...
        print(f"Active Trades Analyzed: {len(trades)}")
        ...
        print(f"  Std Dev Simulated: {simulated_returns.std()*100:.2f}%")
    ...
    def sensitivity_analysis(self, stock_y, stock_x, market_index):
        ...
        if len(results) == 0:
            print("  No sensitivity results - all parameter combinations failed.")
            self.validation_results['sensitivity'] = pd.DataFrame()
            return pd.DataFrame()
    ...
    def transaction_cost_analysis(self, slippage_bps=5, commission_per_trade=1):
        ...
        original_sharpe = (trades['pnl'].mean() / trades['pnl'].std() * 
                           np.sqrt(252 / max(trades['duration_days'].mean(), 1))) if trades['pnl'].std() > 0 else 0
        ...
        sharpe_after_costs = (trades_with_costs['pnl_after_costs'].mean() / 
                             ...
                             np.sqrt(252 / max(trades['duration_days'].mean(), 1))) if trades_with_costs['pnl_after_costs'].std() > 0 else 0
    ...
    def generate_report(self, save_path='validation_report.txt'):
        ...
        if 'walk_forward' in self.validation_results:
            ...
            if len(wf) > 0 and wf['Return'].mean() > 0 and (wf['Return'] > 0).mean() >= 0.6:
                scores['Walk-Forward'] = 100
            elif len(wf) > 0 and wf['Return'].mean() > 0:
        ...
        if 'stress_test' in self.validation_results:
            st = self.validation_results['stress_test']
            if st is not None and len(st) > 0:
                avg_crisis = st['Strategy_Return'].mean()
                avg_trade_rate = st['Trade_Rate'].mean()
                if avg_crisis > st['Market_Return'].mean() and avg_crisis > -5:
