...
def generate_ultra_conservative_signals(z_scores, regimes, spread_returns,
    ...
    for i in range(len(z_scores)):
        ...
        # CRISIS handling: block NEW entries, but let open positions ride
        # (force-exiting on CRISIS was killing profitable trades mid-way)
        if regime == 0:
            if position != 0:
                # Still apply stop-loss even in CRISIS
                days_held = i - entry_date_idx if entry_date_idx is not None else 0
                stop_loss = (
                    (position == 1 and z < -(entry_z_normal * 2.0)) or
                    (position == -1 and z > (entry_z_normal * 2.0)) or
                    days_held > 30
                )
                if stop_loss:
        ...
        # Entry logic — simplified, removed overly strict trend filter
        if position == 0:
        ...
        # Exit logic
            ...
            exit_conditions = [
                # Normal mean reversion — the profit target
                abs(z) < exit_threshold,
                
                # Stop-loss: z-score moved 2x against us (widened from 1.3x)
                (position == 1 and z < -entry_threshold * 2.0),
                (position == -1 and z > entry_threshold * 2.0),
                
                # Maximum holding period (extended from 20 to 30)
                days_held > 30
            ]
            ...
            if days_held >= min_hold_days:
                ...
                if (position == 1 and z < -entry_threshold * 2.0) or \
                   (position == -1 and z > entry_threshold * 2.0) or \
                   days_held > 30:
                    position = 0
                    entry_date_idx = None
                    entry_z = None
