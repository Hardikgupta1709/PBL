...
ticker_x = 'PNC'
score = 110
params = {'entry_z': 1.67, 'exit_z': 0.5, 'min_hold': 2}
...
try:
    ...
    results = system.run_backtest(
        ...
        entry_z_volatile=params['entry_z'] + 0.4,
        exit_z_volatile=max(0.1, params['exit_z'] - 0.1),
        min_hold_days=params['min_hold'],
        z_score_window=30   # shorter window for BAC/PNC's fast 18-day half-life
    )
...
if overall >= 80 and scores['Monte Carlo'] >= 80:
    ...
    print("  3. Configure Paper_Trading/config.py with BAC/PNC")
    print("  4. Run: python run_paper_trading.py --trade")
    print("  5. Monitor for 90 days")
    
elif overall >= 60:
    print("\n MARGINAL ")
    print("\nOptions:")
    print("  1. Paper trade with small size")
    print("  2. Try entry_z=1.5 for more trades")
    print("  3. Extend date range to 2018-2024")
    
else:
    print("\n FAILED VALIDATION")
    print("\nActions:")
    print("  1. Try entry_z=1.5, z_score_window=20")
    print("  2. Test longer date range: 2018-01-01 to 2024-01-01")
    print("  3. Try alternative pairs: VMC/MLM or CVX/OXY")
