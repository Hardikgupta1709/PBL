"""
Automated Pair Scanner - Finds Good Pairs with Zero Effort
Scans pre-defined stock universe and ranks pairs by quality
Run: python auto_find_pairs.py
"""
...
# Pre-defined stock universe (known liquid, large-cap stocks)
STOCK_UNIVERSE = {
...
def quick_pair_score(y, x):
    """
    Fast pair quality scoring (essential tests only)
    Returns score 0-100
    """
...
def scan_sector(sector_name, stocks, start_date, end_date, min_score=50, max_pairs=10):
    ...
    if len(stock_data) < 2:
        print(f"⚠️ Insufficient data in {sector_name}")
        return []
    ...
    if results:
        ...
        print(f"\n⚠️ No pairs with score >= {min_score} in {sector_name}")
...
def display_results(results):
    ...
    if len(acceptable) > 0:
        print(f"\n⚠️ ACCEPTABLE Pairs (Score 50-59): {len(acceptable)}")
    ...
    if len(excellent) > 0:
        ...
        print(f"\n⚠️ Use conservative parameters:")
        print(f"   entry_z_normal = 2.5")
        print(f"   exit_z_normal = 0.3")
        
    else:
        print("\n⚠️ Only marginal pairs found. Consider:")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("\nThis will scan hundreds of stock pairs and rank them by quality.")
    print("Grab a coffee - this takes 10-15 minutes! ☕\n")
    ...
    if df is not None:
        ...
        print(f"   2. Edit test_ultra_conservative.py with those tickers")
        print(f"   3. Run: python test_ultra_conservative.py")
        ...
        print("   python auto_find_pairs.py --min-score 40")