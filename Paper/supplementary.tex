...
\textbf{Clarification on Kalman noise multipliers:} The framework uses\emph{two} distinct multiplier schedules, each with a different purpose:
\begin{itemize}
  \item \textbf{Kalman filter process noise} (\texttt{var\_eta} in code): multipliers $(1, 2, 5)$ for (Normal, Volatile, Crisis). The Crisis multiplier of $5\times$ allows the Kalman filter to adapt very rapidly during extreme regimes.
    \item \textbf{Signal-generation thresholds} (Eq.\,(10) in paper): entry/exit thresholds are regime-specific 
    ($(1.5,0.5)$ for Normal, $(2.0,0.3)$ for Volatile, and no new entries in Crisis). This mechanism is separate from Kalman state estimation.
\end{itemize}
Eq.~(4) in the main paper describes the Kalman process-noise schedule $(1,2,5)$, fully consistent with the implementation.
