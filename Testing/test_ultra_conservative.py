"""
Quick Test: Ultra-Conservative Strategy
This version should achieve p-value < 0.05 in Monte Carlo
Run: python test_ultra_conservative.py
"""

import sys
print("="*80)
print("TESTING ULTRA-CONSERVATIVE STRATEGY")
print("="*80)
...
# Step 3: Monte Carlo Test (Most Important)
print("\n" + "="*80)
print("STEP 3: Monte Carlo Test")
print("="*80)
print("⏱️  Running 1000 simulations... (this may take 2-3 minutes)\n")
...
try:
    ...
    if p_value < 0.05:
        ...
        print(f"  ⚠️ MARGINAL: p = {p_value:.4f} < 0.10")
    ...
    if actual_return > ci_upper:
        print(f"\n  🎉 Your strategy beats 97.5% of random permutations!")
    elif actual_return > simulated_mean:
        print(f"\n  👍 Your strategy beats average random performance")
    else:
        print(f"\n  ⚠️ Your strategy underperforms random shuffling")
...
try:
    ...
    if stress_results is not None and len(stress_results) > 0:
        ...
        print(f"  ⚠️ No crisis periods in data")
        stress_score = 50
except:
    stress_score = 0
...
try:
    ...
    if retention > 0.7:
        ...
        print(f"  ⚠️ MARGINAL: Retains {retention*100:.1f}% after costs")
        tc_score = 60
    else:
...
if mc_score >= 80:
    ...
    if overall >= 80:
        ...
        print("\n⚠️ Monte Carlo passed but other tests need work")
        print("Review stress test and transaction cost results")
        
elif mc_score >= 60:
    print("\n⚠️ MARGINAL - Monte Carlo shows weak significance")
    print("\nConsider:")
...
with open(filename, 'w') as f:
    f.write("ULTRA-CONSERVATIVE STRATEGY TEST RESULTS\n")
    f.write("="*80 + "\n\n")
